...
export default function CapturePage() {
  ...
  const zone = useMemo(() => {
    return ZONE_DEFINITIONS.find((z) => z.uuid === zoneUuid);
  }, [zoneUuid]);
  ...
  }, [loading, player, game, playerTeamId, router, gameId]);
  ...
  useEffect(() => {
    ...
    if (!zone) {
      setStatus('invalid');
      const timeout = setTimeout(() => router.push(gamePath), 3000);
  ...
  }, [loading, game, router, zone, gameId]);
  ...
  const handleCapture = useCallback(async () => {
    ...
    try {
      ...
      setTimeout(() => router.push(`/game/${gameId}`), 3000);
    }
  }, [captureZone, game, router, zoneId, gameId]);
